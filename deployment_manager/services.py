...
class ServiceManager:
    ...
    def __init__(self, project_root: Path, nssm_path: Optional[str] = None,
        ...
        try:
            ...
            if not Path(exe_path).is_absolute():
                if 'php' in exe_path.lower():
                    ...
                    exe_path = subprocess.check_output(['where', 'php'], text=True, shell=True).strip().split('\n')[0]
                else:
                    exe_path = str(self.project_root / exe_path)
            
            app_dir = str(self.project_root / config['dir']) if config['dir'] else str(self.project_root)
            ...
            self._run_nssm(['set', service_name, 'AppStdout', str(logs_dir / 'output.log')])
            self._run_nssm(['set', service_name, 'AppStderr', str(logs_dir / 'error.log')])
    ...
    def uninstall_service(self, service_key: str, stop_first: bool = True) -> tuple[bool, str]:
        """Uninstall a service."""
        if service_key not in self.SERVICES:
            return False, f"Unknown service: {service_key}"
